			...
			switch (growthItem) {
				...
				case "gain-energy": {
					...
					var customScalingIcon =
						scaling_entity !== undefined
							? "<icon class='" + scaling_entity + "'></icon>"
				...
				case "add-token": {
					...
					growthIcons =
						tokenReqOpen + "<wrap>" + tokenIcons + "</wrap>" + tokenRange + tokenReqClose;
...
function dynamicCellWidth() {
	...
	let borderPixels = 0;
	for (const borderWidth of growthBorders.map((x) =>
		x.getAttribute("double") === undefined ? 7 : 11
	)) {
	...
	for (i = 0; i < growthTables.length; i++) {
		...
		if (totalWidth > 1000 || i == 0) {
			...
			weightedSmallCellWidth =
				parseFloat(remainingCellWidth.replace(/px/, "")) /
