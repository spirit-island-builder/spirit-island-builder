...
{#each powerCards.cards as card, i (card.id)}
  <h6
    ...
    <span on:click={showOrHideSectionSubsection(card)}>
      {#if card.isVisible}
        <ion-icon id="form" on:click={showOrHideSectionSubsection(card)} name="chevron-down-outline" />
      {:else}
        <ion-icon id="form" on:click={showOrHideSectionSubsection(card)} name="chevron-up-outline" />
      {/if}
    </span>
  </h6>
  {#if card.isVisible}
